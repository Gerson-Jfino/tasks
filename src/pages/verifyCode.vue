...
const submitLogin = () => {
  loading.value = true
  disabled.value = true
  api.post("/users/verifyCodeSent", form.value)
    .then((res) => {
    ...
    <VCard class="auth-card pa-4 pt-7" width="448" :loading="loading" :disabled="disabled">
      ...
      <VCardText class="pt-2">
        <h5 class="text-h5 mb-1">
          Valide o código recebido
        </h5>
        <p class="mb-0">
          Por favor introduza o código enviado para <b>{{ email }}</b>
        </p>
      </VCardText>
      ...
      <VCardText>
        <VForm @submit.prevent="submitLogin()">
          <VRow>
            <!-- email -->
            <VCol cols="12">
              <VTextField v-model="form.code" autofocus label="Código" type="text" />
            </VCol>
